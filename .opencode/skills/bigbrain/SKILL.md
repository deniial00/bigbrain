---
name: bigbrain
description: Automatically route non-trivial software-engineering work through rigorous playbooks, workflow skills, engineering principles, delegation, and verification. Use before planning or editing features, bug fixes, refactors, performance work, investigations, prototypes, reviews, or multi-step engineering tasks.
license: MIT
...
Read `references/opencode.md` before applying this skill. It translates the remaining Cursor-specific runtime wording without changing the engineering rules.
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the Authoring a skill playbook and OpenCode's Agent Skills format.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → inspect the diff directly. Apply **no-comments** to code comments and **unslop** to prose.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → drive the real surface with the tools available in OpenCode. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception. Never claim live verification when no suitable driver exists.
...
**Use the native `bigbrain` subagent for any implementation delegate spawned inside a playbook step.** It loads this same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) may choose OpenCode's `general`, `explore`, or `scout` subagents for their specialized roles. Preserve that intent.

**Defaults for every delegated task.** Run independent work concurrently when OpenCode supports it. Pass file pointers instead of inlining large context. Inherit OpenCode's configured model unless a native per-agent model is already configured. Never pass an unverified Cursor model slug. For a panel, prefer distinct configured models or agents. If only one model is available, preserve independent prompts and disclose the reduced diversity.
